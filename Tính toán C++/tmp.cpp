...
void randArray(int a[], int n) {
	cout << "Mang gom " << n << " phan tu sau khi phat sinh ngau nhien la:\n";
	for (int i = 0; i < n; i++) {
		a[i] = 10 + rand() % 91;
		cout << a[i] << " ";
	}
...
void count_odd_even(int a[], int n) {
	...
	cout << "So luong so chan: " << cnt_even << "\n";
...
void print_prime_array_2(int a[], int n) {
	...
	else cout << "Khong ton tai so nguyen to!\n";
}
bool div_3(int n) {
	int sum = 0;
	while (n > 0) {
...
void find_div_3(int a[], int n) {
	for (int i = 0; i < n; i++) {
		if (div_3(a[i])) {
			cout << "So chia het cho 3 dau tien la: " << a[i] << endl;
			break;
		}
	}
}
void find_last_prime(int a[], int n) {
	for (int i = n - 1; i >= 0; i--) {
		if (checkPrime(a[i])) {
			cout << "So nguyen to cuoi cung trong mang la: " << a[i] << endl;
			break;
		}
	}
}
...
void print_symmetry_2(int a[], int n) {
	...
	if (cnt_sym > 0) {
		cout << "Cac so doi xung la: ";
		print_symmetry_1(a, n);
	}
	else cout << "Khong ton tai so doi xung\n";
}
bool div_5(int n) {
	int sodu = n % 10;
	if (sodu == 0 || sodu == 5)
...
void find_div_5(int a[], int n) {
	for (int i = 0; i < n; i++) {
		if (div_5(a[i])) {
			cout << "So chia het cho 5 dau tien la: " << a[i] << endl;
			break;
		}
	}
}
...
bool check_left_sum(int a[], int n) {
	for (int i = 2; i < n; i++) {
		if (a[i] == a[i - 1] + a[i - 2])
			return 1;
	}
...
int count_left_sum(int a[], int n) {
	int cnt = 0;
	for (int i = 2; i < n; i++) {
		if (check_left_sum(a, n))
			cnt++;
	}
	return cnt;
}
void print_left_sum(int a[], int n) {
	for (int i = 0; i < n; i++) {
		if (check_left_sum(a, n))
			cout << a[i] << " ";
	}
...
void print_left_sum_2(int a[], int n) {
	int cnt_left_sum = count_left_sum(a, n);
	if (cnt_left_sum > 0) {
		cout << "Cac so co tong bang 2 so lan can trai la: ";
		print_prime_array(a, n);
	}
	else cout << "Khong ton tai so co tong bang 2 so lan can trai!\n";
}
int gcd(int &a, int &b) {
	if (a == 0 || b == 0)
		return  a + b;
...
void print_gcd_array(int a[], int n) {
	int res = a[0];
	cout << "Uoc chung lon nhat cac phan tu trong mang: ";
	for (int i = 0; i < n - 1; i++) {
		gcd(a[i], res);
	}
	cout << res << endl;
...
void print_odd_array_2(int a[], int n) {
	...
	else cout << "Khong ton tai so toan le!\n";
}
...
void print_odd_head_array_2(int a[], int n) {
	...
	if (cnt_odd_head > 0) {
		cout << "Cac so co chu so le dau tien la: ";
		print_odd_head_array(a, n);
	}
	else cout << "Khong ton tai so co chu so le dau tien!\n";
}
int main() {
	srand(time(NULL));
	int a[31];
	cout << "So n sau khi phat sinh ngau nhien la: ";
	int n = random();
	cout << n << endl;
	randArray(a, n);
	...
	find_div_5(a, n);
	cout << "so nguyen x sao cho khoang [-x;x] chua toan bo phan tu mang la: ";
	cout << find_x(a, n) << endl;
	// print_left_sum_2(a, n);
	int cnt1 = 0;
    for(int i = 2; i < n; i++) {
        if(a[i] == a[i-1] + a[i-2]) {
            cout << a[i] << " ";
    ...
    if(cnt1 == 0) {
        cout << "Khong ton tai so co tong bang 2 so lan can trai!\n";
    }
	print_gcd_array(a, n);
	print_odd_array_2(a, n);
	cout << "So chu so toan chan: ";
	cout << count_even(a, n);
	int cnt2 = 0;
	for(int i = 1; i < n - 1; i++) {
